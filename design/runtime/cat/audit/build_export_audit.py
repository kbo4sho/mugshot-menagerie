#!/usr/bin/env python3
"""Localize, harmonize, export, and audit Sassy Cat v1."""
...
CAT = ROOT / "design/runtime/cat"
AUDIT = CAT / "audit"
CHROMA = CAT / "chroma"
ALPHA = CAT / "alpha"
PUBLIC = ROOT / "public/masks/cat"
PAGES = ROOT / "github-pages/public/masks/cat"
...
def main() -> None:
    ...
    if any(image.size != CANVAS for image in generated.values()):
        raise RuntimeError("All generated cat state masters must be 1254 x 1254")
    ...
    # silhouette, tabby field, whiskers, muzzle, lighting, and keyed perimeter.
    blink_mask = feathered_mask(
        CANVAS,
        ((260, 545, 558, 835), (696, 545, 994, 835)),
        blur=18,
    )
    roar_mask = feathered_mask(
        CANVAS,
        ((525, 835, 729, 1087), (303, 412, 542, 579), (712, 412, 951, 579)),
        blur=18,
    )
    ...
    if chosen_side is None or chosen_quality is None:
        raise RuntimeError("No q94-95 cat export stayed under 350KB")
    ...
    # Zoom the whiskers and keyed cheek perimeter on both high-contrast fields.
    whisker_images: list[Image.Image] = []
    whisker_labels: list[str] = []
    for state in STATES:
        for background_name, background in (("white", "#ffffff"), ("black", "#000000")):
            ...
            whisker_images.append(crop.resize((380, 380), Image.Resampling.LANCZOS))
            whisker_labels.append(f"{state} whiskers / {background_name}")
    labeled_sheet(whisker_images, whisker_labels, 3, 380).save(
        AUDIT / f"whisker-matte-detail-{VERSION}.png", optimize=True
    )
    ...
    manifest: dict[str, object] = {
        ...
        "states": {},
        "audit_evidence": [
            f"design/runtime/cat/audit/native-states-{VERSION}.jpg",
            f"design/runtime/cat/audit/native-96-380-states-{VERSION}.png",
            f"design/runtime/cat/audit/hostile-380-states-{VERSION}.png",
            f"design/runtime/cat/audit/whisker-matte-detail-{VERSION}.png",
            f"design/runtime/cat/audit/copy-lighter-crossfades-380-{VERSION}.png",
        ],
    }
    for state, image in masters.items():
