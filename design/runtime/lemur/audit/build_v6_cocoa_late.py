#!/usr/bin/env python3
"""v6: roar-mid is already cocoa through the roar opening so late lerps cannot muddy."""
...
VERSION = "v6"
CAVITY_RGB = (32, 16, 12)
X_LEFT, X_RIGHT = 560, 682
SMILE_Y0, SMILE_Y1 = 978, 996
CHIN_LIMIT = 1064
# Late cream×cocoa is what lost v5. Mid and roar share one modest cocoa mask
# so 0.625–0.750 stay on cavity color without a taller extra-depth bowl.
MID_DEPTH, MID_EDGE = 26.0, 10.0
ROAR_DEPTH, ROAR_EDGE = 26.0, 10.0
CREAM_SHIFT = 18
...
def main() -> None:
    ...
    Image.fromarray(smile.astype(np.uint8) * 255).save(AUDIT / "v6-accepted-smile.png", optimize=True)
    mid_mask.save(AUDIT / "v6-mid-cavity-mask.png", optimize=True)
    roar_mask.save(AUDIT / "v6-roar-cavity-mask.png", optimize=True)
    side_mask.save(AUDIT / "v6-smile-wing-erase-mask.png", optimize=True)
    ...
    mid_rgb = np.asarray(runtime_images["roar-mid"].convert("RGB"))
    roar_rgb = np.asarray(runtime_images["roar"].convert("RGB"))
    extra_depth = (roar_rgb.max(axis=2) < 70) & ~(mid_rgb.max(axis=2) < 70)
    mix_625 = np.asarray(semantic_roar_mix(runtime_images, 0.625).convert("RGB"))
    mix_750 = np.asarray(semantic_roar_mix(runtime_images, 0.750).convert("RGB"))
    extra_stats = {
        "px": int(extra_depth.sum()),
        "jaw_0625_mean_rgb": [int(round(v)) for v in mix_625[extra_depth].mean(axis=0)] if extra_depth.any() else [32, 16, 12],
        "jaw_0750_mean_rgb": [int(round(v)) for v in mix_750[extra_depth].mean(axis=0)] if extra_depth.any() else [32, 16, 12],
        "mid_vs_roar_max_delta": int(np.abs(mid_rgb.astype(np.int16) - roar_rgb.astype(np.int16)).max()),
    }
    manifest = {
        "animal": "lemur",
        "name": "Ringtail Lemur",
        "version": VERSION,
        "generation_route": (
            "v1/v2 ImageGen identity preserved; roar-mid already uses the full roar "
            "cocoa mask from the smile rim so helper 0.625–0.750 cannot mix cream into "
    ...
    print(json.dumps({
        ...
        "extra_depth": manifest["extra_depth"],
