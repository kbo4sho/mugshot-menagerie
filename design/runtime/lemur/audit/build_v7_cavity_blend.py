#!/usr/bin/env python3
"""v7: two-size smile-rim opening composited with cream×cavity snap, not RGB lerp."""
...
def main() -> None:
    ...
    Image.fromarray(smile.astype(np.uint8) * 255).save(AUDIT / "v7-accepted-smile.png", optimize=True)
    mid_mask.save(AUDIT / "v7-mid-cavity-mask.png", optimize=True)
    roar_mask.save(AUDIT / "v7-roar-cavity-mask.png", optimize=True)
    side_mask.save(AUDIT / "v7-smile-wing-erase-mask.png", optimize=True)
    ...
    manifest = {
        ...
        "version": VERSION,
        "generation_route": (
            "v1/v2 ImageGen identity preserved; v5 two-size smile-rim cocoa opening "
            "plus cream×cavity snap in mix_rendered_mask_images / blendRenderedMaskSample; "
            "no new ImageGen"
        ),
