#!/usr/bin/env python3
"""v6: keep the mid cavity on the closed-lip seat so early blends cannot ghost a lower oval."""
...
CAVITY_RGB = (64, 16, 10)
# Neutral W-smile is a thin dark stroke near y=1144. Mid must occupy those
# pixels, not a bowl below them, or 0.125–0.375 ghosts a second mouth.
CAVITY_TOP = 1134
MID_BOX = (548, CAVITY_TOP, 706, 1158)
ROAR_BOX = (540, CAVITY_TOP, 714, 1174)


def smile_slit(neutral: Image.Image, blur: float) -> Image.Image:
    """Cavity occupies the closed W-stroke, not a bowl painted under it."""
    array = np.asarray(neutral.convert("RGBA"))
    rgb = array[..., :3]
    alpha = array[..., 3]
    yy, xx = np.ogrid[:CANVAS[1], :CANVAS[0]]
    roi = (xx >= 560) & (xx < 690) & (yy >= 1128) & (yy < 1148) & (alpha > 128)
    stroke = roi & (rgb.max(axis=2) < 96)
    mask = Image.fromarray(stroke.astype(np.uint8) * 255)
    mask = mask.filter(ImageFilter.MaxFilter(7)).filter(ImageFilter.GaussianBlur(blur))
    return mask
...
def main() -> None:
    ...
    mid_mask = smile_slit(neutral, 1.0)
    roar_mask = ImageChops.lighter(mid_mask, ellipse_cavity(ROAR_BOX, 1.2))
    ...
    manifest: dict[str, object] = {
        ...
        "generation_route": (
            "deterministic v6 lip-seat repair: roar-mid darkens the actual W-stroke; "
            "roar is that same cavity grown downward; no lower shadow ring; no new ImageGen"
        ),
        "runtime_export": {
            "side_px": 1254,
            "quality": 95,
